...
/** validator.js `isURL` options (class-validator does not re-export the type). */
type IsUrlOptions = Parameters<typeof IsUrl>[0];

interface IUrlFieldOptions extends IStringFieldOptions {
  /** Options forwarded to validator.js `isURL` (e.g. allow localhost remotes). */
...
export function URLField(
  options: Omit<ApiPropertyOptions, 'type'> & IUrlFieldOptions = {},
): PropertyDecorator {
  const { urlOptions, ...stringOptions } = options;

  // NOTE: `each` must follow the option — hardcoding `{ each: true }` made every
  // non-array URL fail validation ("each value in remoteEntry must be a URL").
  const decorators = [
    StringField(stringOptions),
    IsUrl(urlOptions ?? {}, { each: stringOptions.each }),
  ];

  if (stringOptions.nullable) {
    decorators.push(IsNullable({ each: stringOptions.each }));
  } else {
    decorators.push(NotEquals(null, { each: stringOptions.each }));
  }
