...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private handleHttpException(exception: HttpException): ErrorDto {
    ...
    const r =
      typeof response === 'object' && response !== null
        ? (response as { errorCode?: ErrorCode; message?: string })
        : undefined;

    // Lets `new ConflictException({ errorCode: ErrorCode.E005 })` surface a
    // machine-readable code, mirroring handleValidationException().
    ...
    const errorRes = {
      ...
      error: STATUS_CODES[statusCode],
      errorCode: errorCode
        ? Object.keys(ErrorCode)[Object.values(ErrorCode).indexOf(errorCode)]
        : undefined,
      message:
        r?.message ||
        (errorCode
          ? this.i18n.t(errorCode as unknown as keyof I18nTranslations)
          : exception.message),
  ...
  private handleQueryFailedError(error: QueryFailedError): ErrorDto {
    ...
    // A `UQ_`-prefixed constraint violation is an expected client error mapped
    // to 409 (a duplicate), not a server fault.
    const isConflict = !!r.constraint?.startsWith('UQ');
    const { status, message } = isConflict
      ? {
    ...
    // Logging a duplicate at error level would pollute production alerting.
    if (isConflict) {
